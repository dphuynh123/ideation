...
import { 
    ...
    HistoryIcon, ZoomInIcon, ZoomOutIcon, MaximizeIcon, XIcon
...
const App: React.FC = () => {
  ...
  const [theme, setTheme] = useState<'light' | 'dark'>('light'); // Default to light to match design
  ...
  const handleGenerate = async (userInput: UserInput) => {
    ...
    //   const data = await generateBusinessMindMap(userInput, language);
      ...
      setIsModalOpen(true); // Re-open modal if error
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className={clsx(`relative w-screen h-screen overflow-hidden bg-white ${theme == 'dark' && `dark:bg-slate-900`}  text-slate-900 dark:text-slate-100 font-sans selection:bg-yellow-200 dark:selection:bg-cyan-900`)}>
        ...
        <header className={clsx(`absolute top-0 left-0 right-0 z-20 h-16 bg-white/80 ${theme == 'dark' && `dark:bg-slate-900/80`} backdrop-blur-md border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6`)}>
            ...
            {/* <nav className="hidden md:flex items-center gap-8">
                <a href="#" className="text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition">{t('navMyProjects')}</a>
                <a href="#" className="text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition">{t('navTemplates')}</a>
                <a href="#" className="text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition">{t('navExport')}</a>
            </nav> */}
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-30 flex items-center gap-4">
             {/* <div className="bg-white dark:bg-slate-800 rounded-full shadow-lg border border-slate-200 dark:border-slate-700 p-1.5 flex items-center gap-1">
                <button className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full text-slate-600 dark:text-slate-300 transition" title={t('zoomOut')}>
                    <ZoomOutIcon className="h-5 w-5" />
                </button>
                <span className="text-xs font-bold w-12 text-center text-slate-700 dark:text-slate-300">100%</span>
                <button className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full text-slate-600 dark:text-slate-300 transition" title={t('zoomIn')}>
            ...
            >
             ...
             {/* <div className="bg-white dark:bg-slate-800 rounded-full shadow-lg border border-slate-200 dark:border-slate-700 p-1.5">
                <button className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full text-slate-600 dark:text-slate-300 transition" title={t('fitScreen')}>
            ...
            <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
                <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden border border-slate-200 dark:border-slate-700 scale-100 animate-in zoom-in-95 duration-200">
                    <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800">
                         <div>
                            <h2 className="text-xl font-bold text-slate-900 dark:text-white">{t('headerTitle')}</h2>
